...
def inc_quantidade(folha, tipo, count):
...
class Command(BaseCommand):
    ...
    def add_data(self, row):
        ...
        cidade = Cidade.objects.get(nome=row[2], estado=pb)
        orgao = get_orgao(cidade, row[3])
        tipofunc = TipoFuncionario.objects.get(nome=row[4])
        mes = int(row[1])
        ano = int(row[0])
        folha = get_folha(orgao, ano, mes)
        quantidade = inc_quantidade(folha, tipofunc, int(row[5]))
